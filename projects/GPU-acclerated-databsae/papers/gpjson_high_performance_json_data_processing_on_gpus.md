...
#### Leveled Bitmap 구축 방법: 2단계 Carry Index

**depth는 어떻게 아는가?**

쿼리의 최대 depth는 JSONPath를 **정적 분석**해서 미리 결정. 예: `$.users.name` → depth 2까지만 구축.

**병렬 구축의 어려움**:

depth 계산은 앞 문자의 컨텍스트가 필요함. `{`/`[`를 만나면 +1, `}`/`]`를 만나면 -1. 스레드 경계를 넘어가면 내 청크의 시작 depth를 모름 → 직접 병렬화 불가.

**해결책: Newline Index와 동일한 2단계 carry 패턴**

```
1단계 커널: 각 스레드가 자기 청크의 순 depth 변화량만 계산 (절대 depth 필요 없음)
            예시: 청크 내용 "{ foo: { bar" → 순 변화 = +2

prefix sum으로 각 청크의 시작 depth 계산:
            변화량 = [0, +2, +1, +3]  (스레드 4개)
            시작 depth = [0, 0, 2, 3]  (prefix sum)

2단계 커널: 각 스레드가 carry(시작 depth)를 들고 자기 청크를 독립적으로 스캔
            → 구조 문자를 만날 때마다 현재 depth 레벨에 비트 기록
```

String Index를 참조해서 문자열 내부의 `{`, `}` 는 무시.

**결과**: 각 스레드가 독립적으로 Leveled Bitmap 구축 → 완전 병렬화.
...
파일을 **동일 크기 파티션**으로 나눠 순차 처리하되, 다음 파티션 로드와 현재 파티션 처리를 **겹쳐서** 실행:
...
파티션 경계가 JSON 객체 중간을 자를 수 있지만, LD-JSON은 **줄 단위로 객체가 분리**되어 있어서 문제없음. Newline Index로 경계에 걸린 줄을 다음 파티션에서 처리. CUDA 스트림으로 로드(H2D)와 GPU 연산을 다른 스트림에서 동시 실행.
