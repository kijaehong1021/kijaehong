...
인코딩할 문자열: **"AABCA"** (5글자, A×3, B×1, C×1)

**먼저 올바른 디코딩 테이블 구성**:

L=8, 심볼 A(freq=4), B(freq=2), C(freq=2)일 때, 각 심볼의 슬롯 수에 따라
nb(읽을 비트 수)와 nsb(new_state_base)를 정해야 next_state가 항상 [0, L) 범위 안에 있어야 함.

```
A (슬롯 4개, nb=1): next_state = nsb + {0,1}  →  nsb는 짝수여야 범위 안에 있음
B (슬롯 2개, nb=2): next_state = nsb + {0,1,2,3}  →  nsb ∈ {0, 4}
C (슬롯 2개, nb=2): next_state = nsb + {0,1,2,3}  →  nsb ∈ {0, 4}

state │ symbol │ nb │ nsb │ next_state 범위
──────┼────────┼────┼─────┼────────────────
  0   │   A    │  1 │  0  │  {0, 1}
  1   │   B    │  2 │  0  │  {0, 1, 2, 3}
  2   │   A    │  1 │  2  │  {2, 3}
...
각 state가 디코딩하는 심볼:
  ...
  B-슬롯: 1, 5        (2개)
  C-슬롯: 3, 7        (2개)
→ 어떤 state에 있느냐가 어떤 심볼을 출력할지를 결정
```
...
"AABCA" 5글자를 인코딩한 결과:
```
헤더에 기록: initial_state = 4
비트스트림 (왼쪽이 MSB): 0 0 1 1 1 0   (6비트)
```

비트스트림 레이아웃을 오른쪽부터 그룹으로 나누면:
```
← 먼저 소비   나중에 소비 →
[ 0 ] [ 1 ] [ 1 1 ] [ 0 0 ]
  ↑     ↑      ↑       ↑
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Step 1: state=4
  테이블[4] → symbol=A   →   'A' 출력  ← A-슬롯이므로 A
  nb=1, nsb=4
  read_bits(1) = 0   (비트스트림 맨 오른쪽 1비트)
  next_state = nsb + bits = 4 + 0 = 4
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Step 2: state=4
  테이블[4] → symbol=A   →   'A' 출력  ← 여전히 A-슬롯
  nb=1, nsb=4
  read_bits(1) = 1
  next_state = 4 + 1 = 5   ← B-슬롯으로 이동!
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Step 3: state=5
  테이블[5] → symbol=B   →   'B' 출력  ← B-슬롯이므로 B
  nb=2, nsb=4
  read_bits(2) = 11 (=3)
  next_state = 4 + 3 = 7   ← C-슬롯으로 이동!
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Step 4: state=7
  테이블[7] → symbol=C   →   'C' 출력  ← C-슬롯이므로 C
  nb=2, nsb=4
  read_bits(2) = 00 (=0)
  next_state = 4 + 0 = 4   ← A-슬롯으로 이동

  남은 비트스트림: (없음)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Step 5: state=4
  테이블[4] → symbol=A   →   'A' 출력  ← 마지막 심볼

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
디코딩 결과: A A B C A = "AABCA"  ✓
```
...
어느 슬롯에 있느냐 = 어떤 심볼이 나오는지 결정
  state 4 = A-슬롯 → A 출력
  state 5 = B-슬롯 → B 출력
  state 7 = C-슬롯 → C 출력

읽은 비트 = 다음 state 결정
  state 4에서 bit=1 읽음 → state 5 (B-슬롯)로 이동
  state 5에서 bits=11 읽음 → state 7 (C-슬롯)로 이동
  state 7에서 bits=00 읽음 → state 4 (A-슬롯)로 복귀

즉, 인코더가 비트스트림에 "다음에 어느 슬롯으로 갈지"를 기록해둔 것
```

**압축률 확인**:

```
원본: "AABCA" = 5글자 × 8비트 = 40비트 (ASCII)
         또는 5글자 × 3비트 (고정 코드) = 15비트

FSE 압축: 6비트 + 헤더(state=4, 3비트) = 9비트
평균: 6비트 / 5심볼 = 1.2비트/심볼

이론 엔트로피 (이 특정 문자열 기준, A=3/5, B=1/5, C=1/5):
  H = -(3/5×log2(3/5) + 1/5×log2(1/5) + 1/5×log2(1/5))
    ≈ 1.37비트/심볼

1.2비트 < 1.37비트 → 이론값보다 좋아 보이지만,
이는 짧은 예시라 헤더 오버헤드가 비율에 안 잡힌 것.
실제 긴 데이터에서는 이론값에 0.1% 이내로 근접.
```
...
**근본적인 문제: 시퀀스 위치를 사전에 알 수 없다**

```
LZ4 압축 스트림은 가변 길이 시퀀스의 연속:
  [token][literals][offset][token][literals][offset]...

token 상위 4비트를 봐야 literal 길이를 알고
→ literal 길이를 알아야 offset이 어디 있는지 알고
→ 그래야 다음 시퀀스가 어디서 시작하는지 앎

즉, seq_N의 압축 스트림 내 시작 위치는 seq_0 ~ seq_N-1을 파싱해야만 알 수 있음
→ 32개 스레드가 각자 자기 시퀀스로 "점프"하는 건 불가능
```

출력 위치도, 입력(압축 스트림 내) 위치도 둘 다 순차적으로만 알 수 있음. 이게 LZ 병렬화의 근본 장벽.

**실제 warp 협력 방식 1: 파서 + 복사 분리**

```
역할 분리:
  thread 0 (파서):   압축 스트림을 순차 파싱 → 작업 큐에 기록
  thread 1~31 (복사): 큐에서 작업을 꺼내 실제 복사 수행

thread 0 파싱 결과 (작업 큐):
  [literal_src=100, literal_len=12, dst=0  ]   ← seq0
  [match_src=0,     match_len=11,  dst=12  ]   ← seq1
  [match_src=11,    match_len=6,   dst=23  ]   ← seq2
  ...

thread 1~31:
  literal 복사   → 완전 병렬 (서로 다른 구간)
  match 복사     → 의존성 없는 것부터 병렬 실행
  → 파싱(순차)과 복사(병렬)를 파이프라이닝
```

**실제 warp 협력 방식 2: 사전 스캔 후 배분 (2-pass)**

```
패스 1 (단일 스레드, 순차):
  압축 스트림 전체를 빠르게 스캔
  → 각 시퀀스의 (압축 스트림 내 위치, 출력 위치) 테이블 생성
  input_offsets:  [0, 15, 28, 41, ...]   ← 각 시퀀스가 압축 스트림 내 어디서 시작하는지
  output_offsets: [0, 12, 23, 28, ...]   ← 각 시퀀스가 출력 버퍼 어디에 쓰는지

패스 2 (warp 전체, 병렬):
  테이블을 보고 thread i → seq_i의 위치로 직접 점프
  → 이제 입력/출력 위치 모두 알기 때문에 진짜 독립 병렬 처리 가능

thread 0: compressed[0..14]  파싱 → output[0..11]  기록
thread 1: compressed[15..27] 파싱 → output[12..22] 기록  (동시!)
thread 2: compressed[28..40] 파싱 → output[23..27] 기록  (동시!)
```

이 방식에서 패스 1 스캔이 overhead가 되지만, 패스 2 복사가 워낙 무거워서 전체적으로 이득.

**warp shuffle로 출력 위치 prefix sum 수행**:

```
패스 1 스캔에서 각 시퀀스의 출력 크기만 수집:
  output_size: [20, 11, 17, ..., 9]

warp 전체가 동시에 prefix sum 계산 (__shfl_up_sync):
